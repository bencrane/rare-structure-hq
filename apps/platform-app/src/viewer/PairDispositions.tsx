/**
 * PairDispositions — the shared pair-disposition review surface.
 *
 * One parameterized component behind the nine industry-bucket viewer tabs
 * (Fire, Food, IMC, MedDev, MedStaff, Staffing, Textiles, Transpo, WasteEnv).
 * Each thin per-industry wrapper (e.g. FirePairs.tsx) supplies its baked
 * dataset, heading, localStorage key, and category short-name/color maps —
 * everything else (sticky totals bar, disposition chips, overrides, "Copy
 * decisions" export, collapsed sub-$10M tail) is identical across buckets.
 *
 * Candidate universe per bucket: every (award-NAICS6, award-PSC4) pair
 * observed FY23–25 under that bucket's scope. Each row shows its proposed
 * disposition and reason; chips reassign (persisted per-bucket in
 * localStorage); "Copy decisions" exports the final lists.
 */
...
export type PairDispositionsData = {
  title: string;
  window: string;
  artifact: string;
  scope: string;
...
export type PairDispositionsProps = {
  /** The baked dataset (cast from the per-industry JSON import). */
  data: PairDispositionsData;
  /** h1 text, e.g. "Wildland Fire & Forestry Services — pair dispositions". */
  heading: string;
  /** localStorage key for overrides, e.g. "viewer.fire-pairs.v1". */
  storageKey: string;
  /** Category → chip label (include "Out"). */
  short: Record<string, string>;
  /** Category → chip color (include "Out"). */
...
function loadOverrides(storageKey: string): Overrides {
  try {
    return JSON.parse(localStorage.getItem(storageKey) ?? "{}") as Overrides;
  } catch {
    return {};
  }
}
...
export function PairDispositions({
  ...
  short,
  catColor,
}: PairDispositionsProps) {
  const [overrides, setOverrides] = useState<Overrides>(() => loadOverrides(storageKey));
  ...
  const totals = useMemo(() => {
    ...
    for (const p of data.pairs) {
      const b = overrides[pairKey(p)] ?? p.bucket;
      t[b] ??= { pairs: 0, oblM: 0 };
      t[b].pairs += 1;
      t[b].oblM += p.obl_m;
    }
  ...
  const copyDecisions = () => {
    ...
    navigator.clipboard
      .writeText(
        JSON.stringify({ artifact: data.artifact, window: data.window, decisions }, null, 1),
      )
      .then(() => {
        setCopied(true);
      ...
      >
        {data.categories.map((cat) => (
          <span key={cat} style={{ fontSize: 13 }}>
            <span style={{ color: catColor[cat] ?? "#333", fontWeight: 700 }}>
              ● {short[cat] ?? cat}
            </span>{" "}
            <span style={mono}>
                  ...
                  {data.categories.map((cat) => (
                    <button
                      ...
                      style={{
                        ...
                        cursor: "pointer",
                        border: `1px solid ${cur === cat ? (catColor[cat] ?? "#333") : "#ddd"}`,
                        background: cur === cat ? (catColor[cat] ?? "#333") : "#fff",
